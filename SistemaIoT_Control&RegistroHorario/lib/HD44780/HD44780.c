...
void LCD_init(uint8_t addr, uint8_t dataPin, uint8_t clockPin, uint8_t cols, uint8_t rows)
{
    ...
    LCD_rows = rows;
    I2C_init();
    vTaskDelay(pdMS_TO_TICKS(200));                                 // Initial 40 mSec delay
    ...
    ets_delay_us(100);                                                  // 100 uS delay (min)
    ...
    ets_delay_us(40);                                                   // 40 uS delay (min)
    ...
    vTaskDelay(pdMS_TO_TICKS(4));                                   // Clearing memory takes a bit longer
